...
function getYouTubeUrl(article, locationState) {
  if (locationState?.url) return locationState.url
  if (locationState?.youtube_url) return locationState.youtube_url
  if (article?.youtube_url) return article.youtube_url
  if (article?.video_url) return article.video_url
  if (article?.url) return article.url
  if (article?.youtube_id) return `https://www.youtube.com/watch?v=${article.youtube_id}`
  return null
}
